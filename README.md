...
First snapshot captured 2026-07-09 (1,987 names; Mid+Large+Mega, US primary). Ingest and the Norgate merge run on live data; the forward-return analysis is pre-registered and **gate-locked** (a coded refusal below 8 weekly captures; verdicts need ≥ 26 captures and ≥ 20 matured 1-month cohorts). The scoring & selection framework was **frozen 2026-07-10** — five graded schemes, shared construction conventions, KEEP bar and graduation rule (`RESEARCH_MEMO.md`, register rows 3–5); `analyse.py` implements it (register rows 6–9) and was **pre-flight-verified 2026-08-13** against the register with the full path proven end to end on synthetic fixtures — planted alpha recovered exactly, delisting realised, retention band and episode costs on schedule (`reviews/preflight_2026-08-13.md`; re-run with `python scripts/fixture_check.py`). See `RESEARCH_MEMO.md`.
...
`capture.py` is the guard layer. It validates the export *before* anything is filed or ingested — column contract (imported from `ingest.py`, never copied), row count and ticker overlap against the prior capture, a content hash against the previous export to catch a stale re-submit, and date sanity — aborts on any failure, then files the raw CSV to OneDrive and runs ingest → feed gate → merge → dashboard → HTML export (+ OneDrive copy) → public page. `--latest` takes the newest CSV in either `Downloads` or `OneDrive\Main\tipranks-signal` (already-filed `tipranks_*.csv` archives are skipped, so a prior capture can never be offered back as a fresh export). Flags: `--file <path>` instead of `--latest`, `--validate-only` to run the guards alone, `--force` to overwrite a captured date, `--push` to commit and push the public aggregates. `python scripts/status.py` reports accrual.
