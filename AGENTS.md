# AGENTS.md — tipranks-signal

Inherits the vault `C:\dev\AGENTS.md`. Project-specific rules below; this file overrides where it conflicts.
...
- **Hosted monitor shell (added 2026-07-31).** The monitor's CODE is published at `/monitor/` on Pages; its DATA is not, and the sentence above is unweakened. `docs/monitor/index.html` is `template.html` plus `monitor_shell.html` and carries no vendor values — the operator loads the weekly `data/exports/` export from local disk, it is read in-browser (FileReader → IndexedDB) and never uploaded; the page is static, so there is no endpoint it could be uploaded to. Two guards enforce this and both must stay: `pipeline.py` fails the build if any ticker from any merge appears in the shell, if an `__EXPORT_DATA__` / `__EXPORT_PRICES__` payload is inlined, or if the shell exceeds `MON_MAX_KB`; the pre-commit hook independently rejects a staged `docs/monitor/index.html` over 150 KB. Never inline panel data into that build to "make it easier" — that is the one change this design exists to prevent.
